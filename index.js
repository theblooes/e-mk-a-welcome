...
canvax.registerFont("./storage/New Comic BD Outline.otf", { family: 'Discord' })
canvax.registerFont("./storage/New Comic BD.otf", { family: 'Discordx' })
canvax.registerFont("./storage/New Comic BD Shadow.otf", { family: 'Discordxx' })
...
async function generareCanvas(member) {
  ...
  const background = await resolveImage(await db.get(`bg_${member.guild.id}`)) ?? await resolveImage("https://whatifgaming.com/wp-content/uploads/2022/03/Gamer-Girl-Aesthetic.gif")
